  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={product.name}>
      <div className="grid gap-8 md:grid-cols-[1fr_1fr]">
        ...
        />
        <div>
          <div className="text-sm uppercase tracking-[0.24em] text-roseBrown/70">{product.brand}</div>
          <div className="mt-3">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {product.variants.map((variant) => (
              <span key={variant} className="rounded-full border border-line px-4 py-2 text-sm">
                {variant}
              </span>
            ))}
          </div>
          <div className="mt-6 flex items-end gap-3">
            ...
            {product.oldPrice ? (
              <div className="text-sm text-roseBrown/60 line-through">{formatPrice(product.oldPrice)}</div>
            ) : null}
          </div>
