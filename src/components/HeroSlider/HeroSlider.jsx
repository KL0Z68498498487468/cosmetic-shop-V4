...
const AnimatedNumber = ({ end, duration = 1500, suffix = '', trigger, locale }) => {
...
export default function HeroSection({ recommendations = [], formatPrice }) {
  ...
  const categories = siteTexts.categories;
  ...
  return (
    <section className={`overflow-hidden rounded-2xl transition-colors duration-300 sm:rounded-3xl ${
      ...
      <div className="relative px-5 pb-0 pt-6 sm:px-8 sm:pt-8 lg:grid lg:min-h-[400px] lg:grid-cols-[1fr_340px] lg:items-center lg:gap-6">
          ...
          >
            <div>
              <div className={`text-lg font-bold ${isDark ? 'text-slate-100' : 'text-[#1C1410]'}`}>
                <AnimatedNumber end={4800} suffix="+" trigger={mounted} locale={locale} />
              </div>
              <div className={`mt-0.5 text-[10px] ${isDark ? 'text-slate-500' : 'text-[#9C8880]'}`}>{t('common.clients')}</div>
            </div>
            <div className={`h-5 w-px ${isDark ? 'bg-slate-800' : 'bg-[#E0D5CF]'}`} />
            <div>
              <div className="flex items-center gap-0.5">
                {[...Array(5)].map((_, index) => (
                  <FiStar key={index} size={10} fill={isDark ? '#64748b' : '#C48B72'} stroke="none" />
                ))}
              </div>
              <div className={`mt-0.5 text-[10px] ${isDark ? 'text-slate-500' : 'text-[#9C8880]'}`}>{t('common.ratingLabel')}</div>
            </div>
            <div className={`h-5 w-px ${isDark ? 'bg-slate-800' : 'bg-[#E0D5CF]'}`} />
            <div>
              <div className={`text-lg font-bold ${isDark ? 'text-slate-100' : 'text-[#1C1410]'}`}>
                <AnimatedNumber end={500} suffix="+" trigger={mounted} locale={locale} />
              </div>
              <div className={`mt-0.5 text-[10px] ${isDark ? 'text-slate-500' : 'text-[#9C8880]'}`}>{t('common.brandsCount')}</div>
            </div>
          </div>
        </div>
        ...
        >
          {featured ? (
            <Link
              ...
              className={`group relative z-20 block overflow-hidden rounded-[1.25rem] transition-all duration-300 hover:-translate-y-1 ${
                isDark
                  ? 'bg-slate-800 shadow-[0_8px_30px_rgba(0,0,0,0.3)]'
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {[secondary, tertiary].filter(Boolean).map((product) => (
              <Link
                key={product.id}
              ...
              >
                ...
                <div className="min-w-0">
                  <div className={`truncate text-[11px] font-semibold ${isDark ? 'text-slate-100' : 'text-[#1C1410]'}`}>{product.name}</div>
                  <div className={`text-[10px] font-bold ${isDark ? 'text-slate-500' : 'text-[#6B5C54]'}`}>{formatPrice(product.price)}</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      {categories.length ? (
        <div
          className={`mt-5 grid divide-x border-t transition-all duration-700 sm:grid-cols-3 ${
          ...
          {categories.slice(0, 3).map((category) => (
            ...
            >
              <div className="h-8 w-8 flex-shrink-0 overflow-hidden rounded-lg">
                <img src={category.image} alt={category.title} className="h-full w-full object-cover grayscale-[0.3] group-hover:grayscale-0" />
              </div>
              <div className="min-w-0 flex-1">
                <div className={`truncate text-[12px] font-bold ${isDark ? 'text-slate-200' : 'text-[#1C1410]'}`}>{category.title}</div>
                <div className={`truncate text-[10px] ${isDark ? 'text-slate-500' : 'text-[#9C8880]'}`}>{category.description}</div>
              </div>
