...
const Footer = () => {
  ...
  return (
    <footer className="mt-20 border-t border-white/60 bg-white/80">
      <div className="container-shell py-16">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr_0.8fr]">
          <div className="rounded-[2rem] bg-ink bg-soft-mesh p-8 text-white shadow-soft">
          ...
          <div>
            <div className="text-sm font-bold uppercase tracking-[0.25em] text-roseBrown/70">
              Компания
            </div>
            <div className="mt-5 space-y-3">
              {siteTexts.footerLinks.company.map((item) => (
                <Link key={item.to} to={item.to} className="block text-sm transition hover:text-accent">
                  {item.label}
                </Link>
              ))}
            </div>
